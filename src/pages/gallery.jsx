...
import { useLocation as useSchoolLocation , useNavigate  } from "react-router-dom"; // useRoute 대신 useLocation 사용
...
const Gallery = () => {
  ...
  const createImagesArray = () => {
    ...
    for (let i = 0; i < tokenIds.length; i++) {
      ...
      if(i<6){
        ...
        imagesArray.push({
          position: [-5.6 + ((i-8) * 2), 2, -0.6], // 예시 position, 필요한 값으로 변경할 수 있습니다
          rotation: [0, 0, 0], // 예시 rotation, 필요한 값으로 변경할 수 있습니다
          url: arrayOfImageUrls[i],
          country:  CountryValue,
          city: CityValue,
  ...
  return (
    <>
      {loading ? (
        <div>Loading...</div>
      ) : tokenIds.length === arrayOfImageUrls.length && show ? (
        ...
        <Canvas dpr={[1,1.5]} >
          ...
          <Init start={start} setStart={setStart} setOrbitControlsEnabled={setOrbitControlsEnabled}/>
          <Frames  images={images} setOrbitControlsEnabled={setOrbitControlsEnabled} popup={popup}
          setPopup={setPopup} start={start} menu={menu} setMenu={setMenu} setShow={setShow}/>
...
function Frames({
  ...
  popup, setPopup,start,menu, setMenu, setShow,
  ...
  useEffect(() => {
    ...
    if (clicked.current) {
      ...
      clicked.current.parent.localToWorld(p.set(0.7, GOLDENRATIO / 2, 1.25));
  ...
  return (
    <group
      ...
      onClick={(e) => (
        e.stopPropagation(),
        setLocation(
          clicked.current === e.object ? "/gallery" : "/gallery/" + e.object.name
        )
      )}
    ...
    >
      ...
      <div className='flex'>
      <button onClick={()=> setShow(false)} className='text-white px-2 ml-4 mt-4'>
      <AiOutlineHome size={33}/>
      </button>
      <button onClick={() => setPopup(true)}  className='text-white px-2 ml-4 mt-4'>
...
function Frame({menu, p, country, city, address, weather,  url, c = new THREE.Color(), ...props }) {
  const image = useRef();
  const frame = useRef();
  const [, params] = useRoute("/gallery/:id");
  ...
  return (
    <group {...props}>
      <spotLight color="#A1FE00"  position= {[0,2,0]} intensity={50} distance={2.5} angle={Math.PI} decay={1} />      
      <mesh
        name={name}
      ...
      // ref={frame}
      raycast={() => null}
      scale={[0.9, GOLDENRATIO *0.9 , 0.05]}
